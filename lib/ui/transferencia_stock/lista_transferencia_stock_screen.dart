...
class ListaTransferenciaStockScreen extends StatefulWidget {
  const ListaTransferenciaStockScreen({super.key});

  @override
  State<ListaTransferenciaStockScreen> createState() => _ListaTransferenciaStockScreenState();
}

class _ListaTransferenciaStockScreenState extends State<ListaTransferenciaStockScreen> with SingleTickerProviderStateMixin {
  ...
  void cargarSolicitudes(){
    ...
    BlocProvider.of<TransferenciaStockBloc>(context).add(LoadTransferenciaStock(pageNumber: 1, pageSize: 30));
  }

  void cargarSolicitudesBySearch(){
    BlocProvider.of<TransferenciaStockBloc>(context).add(LoadTransferenciaStock(pageNumber: 1, pageSize: 30, search: controllerSearch.text));
  }

  void cargarSolicitudesByDocDate(docDate){
    BlocProvider.of<TransferenciaStockBloc>(context).add(LoadTransferenciaStock(pageNumber: 1, pageSize: 30, docDate: docDate));
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: BlocConsumer<TransferenciaStockBloc, TransferenciaStockState>(
              ...
              builder: (context, state) {
                ...
                } else if (state is TransferenciaStockLoadError) {
                  return Center(
...
class ListViewTransferenciaStock extends StatelessWidget {
  ListViewTransferenciaStock({super.key, required this.solicitudes});
        ...
        return Container(
          ...
          child: ItemListSolicitudTraslado(
            ...
            onOpen: () async {
              ...
              final result = await navigatorContext.push('/detalletransferenciastock', extra: solicitud);
              
              if (result == true) {
                if (navigatorContext.mounted) { // Verificamos si el widget aún está montado
                  BlocProvider.of<TransferenciaStockBloc>(navigatorContext).add(LoadTransferenciaStock(pageNumber: 1, pageSize: 30));
                }
              }
            }
          )
